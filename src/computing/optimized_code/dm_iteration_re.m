function [p_new_til,S_new_til] = dm_iteration_re(p_til,S_til,S_bar_til,sigma_re,nsup1r1,nsup1r2,nsup1r3,z_hat,sigma_bi,tau_re,gamma_re,mu_re,M,N,MN,b)
    ...
    nsup1r3_rshp=reshape(nsup1r3(:,:,5),[1,MN]);
%     % duplication pour le calcul du prox sans boucle
%     nsup1r1_r=repmat(nsup1r1_rshp(1,:)==1,2,1);
%     nsup1r2_r=repmat(nsup1r2_rshp(1,:)==1,2,1);
%     nsup1r3_r=repmat(nsup1r3_rshp(1,:)==1,2,1);
    ...
    norm_nu_2 = repmat(vecnorm(nu_2,2,1),2,1); % norme par colonne
    ...
    norm_nu_3 = repmat(vecnorm(nu_3,2,1),2,1); % norme par colonne
    psi_3=nu_3.*(norm_nu_3-1)./(sigma_bi*norm_nu_3);   

    % pour les pixels hors du co-support, initialisation à z_til
    p_new_til_1=z_til_1(1:2,:);
    p_new_til_2=z_til_2(1:2,:);
    p_new_til_3=z_til_3(1:2,:);
    ...
    for i=1:MN
        ...
        if(nsup1r3_rshp(i)==1)
            p_new_til_3(:,i) = refitting_part(z_til_3(:,i),psi_3(:,i),gamma_re);
        end
    end
%     % calcul du prox sans boucle
%     nb1=sum(nsup1r1_rshp,'all');
%     nb2=sum(nsup1r2_rshp,'all');
%     nb3=sum(nsup1r3_rshp,'all');
...
%             reshape(z_til_2(nsup1r2_r==1),[2,nb2]), ...
%             reshape(psi_2(nsup1r2_r==1),[2,nb2]), ...
%             gamma_re,2);
%     end
%     if ~isempty(p_new_til_3(nsup1r3_r==1))
%         p_new_til_3(nsup1r3_r==1) = refitting_part_2( ...
%             reshape(z_til_3(nsup1r3_r==1),[2,nb3]), ...
%             reshape(psi_3(nsup1r3_r==1),[2,nb3]), ...
%             gamma_re,2);
%     end
    ...
    p_new_til(:,:,5)=reshape(p_new_til_3(1,:),[M,N]);
    p_new_til(:,:,6)=reshape(p_new_til_3(2,:),[M,N]);
